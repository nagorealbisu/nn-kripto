...
Ciphertext<DCRTPoly> forward_pass(
    ...
    for (int layer = 0; layer < nn->n_layers - 1; layer++){
        ...
        if(multdepth - cY->GetLevel() < 4){ // ONDO PENTSATU: sarearen dimentsioen arabera konfiguratu??
...
KeyPair<DCRTPoly> init_crypto_context_ckks_test(CryptoContext<DCRTPoly> &cc, nn_t *nn, uint32_t &numSlots){
...
extern "C" int encrypted_dataset_testing(nn_t *nn, ds_t *ds) {
    ...
    CryptoContext<DCRTPoly> cc;
    KeyPair<DCRTPoly> keys = init_crypto_context_ckks_test(cc, nn, numSlots);
