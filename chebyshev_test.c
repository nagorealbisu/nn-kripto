...
*/
{
  ...
  printf ( "  Test the CHEBYSHEV C library.\n" );
...
void test02 ( void )
{
  ...
  c = chebyshev_coefficients ( a, b, n, sigmoid );

  //Calcular chebysheb de 1 único punto
  x[0] = 0.5;
  
  fc = chebyshev_interpolant ( a, b, n, c, m, x );

  printf ( "\n" );
  printf ( "  F(X) is sigmoid function:\n" );
  printf ( "\n" );
  printf ( "          X               C(I)            F(X)           C(F)(X)\n" );
  printf ( "\n" );
  for ( i = 0; i < m; i++ )
  {
    printf ( "  %14g  %14g  %14g  %14g\n", x[i], c[i], sigmoid(x[i]), fc[i] );
  }
